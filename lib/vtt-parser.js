...
Parser.prototype._handleLine = function(line) {
    if (line.length === 0) {
        ...
        this._currentItem.id = this.previousLine;
...
Parser.prototype._pushCurrentItem = function() {
    if (this._currentItem !== null) {
        if (Array.isArray(this._currentItem.payload)) {
          if (this._currentItem.payload.join('').substr(0, 1) === '<') {
            var results = '';
            var root = true;
            var content = this._currentItem.payload.join('').replace('{', '"{').replace('}', '}"');
            ...
            this._currentItem.payload = JSON.parse(results);
          }
          else {
