...
Parser.prototype._handleLine = function(line) {
    if (line.indexOf('Region: id') === 0) {
        ...
        this._currentItem.id = this.previousLine && this.previousLine.trim() || '_' + this._i;
    ...
    if (this._currentItem !== null) {
        if (this._currentItem.type === 'region') {
          ...
          if (this._currentItem.payload.join('').substr(0, 1) === '<' || this._currentItem.payload.join('').indexOf('>') > -1) {
            ...
            for (var i in items) {
                try {
                    payload.push(eval(jsxjs(items[i])));
                }
                catch(e) {
          ...
          else if (Array.isArray(this._currentItem.payload) && this._currentItem.payload.length) {
